...
Proxy.prototype.name = '__proxy__';
...
pro.start = function(cb) {
  ...
  this.addServers(this.app.get('servers'));
};
...
var genProxies = function(client, app, sinfos) {
  var item;
  for(var serverType in sinfos) {
    item = sinfos[serverType];
     if(hasProxy(client, item)) {
      continue;
    }
    client.addProxies(getProxyRecords(app, serverType));
  }
};
 ...
 */
var getProxyRecords = function(app, serverType) {
  var records = [], appBase = app.getBase(), record;
  // sys remote service path record
  record = pathUtil.getSysRemotePath('backend');
  if(record) {
    records.push(pathUtil.remotePathRecord('sys', serverType, record));
  }
  // user remote service path record
  record = pathUtil.getUserRemotePath(appBase, serverType);
  if(record) {
    records.push(pathUtil.remotePathRecord('user', serverType, record));
  }
  return records;
};
