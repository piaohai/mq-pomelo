...
SessionService.prototype.create = function(id, sid, socket) {
  ...
  this.sessionsMap[sid+id] = this.sessions[session.id] = session;
  return session;
};
...
SessionService.prototype.clone = function(session) {
  ...
  var sid = session.sid;
  if (!id || !sid) return;
  var __session = this.sessionsMap[sid+id];
  if (!!__session) {
    console.log('clone exist %j',session);
    __session.uid = session.uid;
    __session.settings = session.settings;
    return __session;
  } else {
    console.log('clone not exist %j',session);
 ...
 * @api private
 */
SessionService.prototype.unbind = function(id, uid, cb) {
  var session = this.sessions[id];
  if(!session) {
    cb(new Error('session not exist, id: ' + id));
    return;
  }
  delete this.sessionsMap[session.sid+session.id];
