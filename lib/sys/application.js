 ...
 */
Application.init = function(Pomelo,opts) {
  opts = opts || {};
  ...
  Application.loadSync(Pomelo,services);
  Application.loadSync(Pomelo,components);
  Application.loadSync(Pomelo,filters);
  Application.loadSync(Pomelo,modules);

  appUtil.defaultConfiguration(this);

  this.state = STATE_INITED;

  logger.info('application inited: %j', this.get('curServer').id);

};


Application.loadSync = function(Pomelo,compName) {
  Pomelo[compName] = {};
  var comPath = '/'+compName+'/';
  fs.readdirSync(__dirname + comPath).forEach(function (filename) {
    if (!/\.js$/.test(filename)) {return;}
    var name = path.basename(filename, '.js');
    function load() {return require(prefix+comPath + name);}
    Pomelo[compName].__defineGetter__(name, load);
...
Application.protocol = function(serverType,type,protocol) {
  var __protocols = this.get('__protocols__');
  if (type!=='rpc' && type!=='handle' && type!=='socket') {
    throw new Error('wront type protocols' + type);
  }
  if(!__protocols) {
    __protocols = {};
  }
  if (!__protocols[serverType]){
      __protocols[serverType] = {};
  }
  __protocols[serverType][type] = protocol;
  this.set('__protocols__', __protocols);
  return this;
};
 ...
 * @memberOf Application
 */
Application.load = function(component, opts) {
  var _comp = component(this,opts);
  var name = _comp.name;
  ...
  this.loaded.push(_comp);
  if(!!name) {
    this.components[name] = _comp;
  }
  return this;
};
...
Application.getServerById = function(serverId) {
   var map = this.get('__serverMap__');
  return map[serverId];
};
