...
Monitor.prototype.start = function(serverType,serverId) {
  var self = this;
  if (!cluster.isMaster){
    return;
  }
  ...
  console.error('connected %j %j',serverType,serverId);
  setTimeout(function(){
    var client = net.connect(self.masterInfo.port,function(){
    console.error('connected %j %j',self.masterInfo.port,self.masterInfo.host);
    var msg = {route:'master.server.add',id:serverId,serverType:serverType};
  ...
  client.on('data',function(data){
    ...
    if (msg.method==='sync'){
      self.app.syncServers(msg.servers);
    }
  });
  client.on('end', function() {
    ...
    buffer = [];
    var msg = JSON.parse(data+'');
    console.log('endendendend %j',msg);
  });
  },2000);
  return;
