...
var FilterService = require('../sys/service/filterService');
var HandlerService = require('../sys/service/handlerService');
var crc = require('crc');
...
pro.start = function(serverType,serverId) {
  ...
  self.amqp.start(serverType,serverId);
...
pro.stop = function() {
  ...
  self.amqp.stop(serverType,serverId);
...
pro.response = function(err,resp,session,req) {
  ...
  this.afterFilter(err, req, _session,resp,function(err,res){
      ...
      self.app.amqp.connection.publish(req.from,self.encode(package));
  });
};
...
pro.doForward = function(session,msg,routeRecord, cb) {
  ...
  self.app.amqp.connection.publish(serverId,this.encode(package));
};
