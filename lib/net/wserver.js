...
var Wserver = function (app,opts) {
  ...
  this.routes = app.get('__routes__') || {};
  this.protocols = app.get('__protocols__') || {};
...
pro.encode = function(type,msg){
  return this.getProtocol(type).encode(msg);
}

pro.decode = function(type,msg){
  return this.getProtocol(type).decode(msg);
}

pro.getProtocol = function(type){
  var self = this;
  var serverType = self.app.get('serverType');
  if (!self.protocols[serverType]) return defaultProtocol;
  if (!self.protocols[serverType][type]) return defaultProtocol;
  return self.protocols[serverType][type];
}
...
pro.response = function(err,resp,session,req) {
  ...
  this.afterFilter(err, req, _session,resp,function(err,res){
      ...
      var package = {session:_session, request:request,type:REQPONSE};
      //console.error('mq respnserespnserespnse %j ',package);
      self.store.response(req.from,package);
  });
};
...
pro.broadcast = function(err,resp,serverType,session,req) {
  ...
  this.afterFilter(err, req, _session,resp,function(err,res){
      ...
      var package = {session:_session, request:request,type:BROADCAST};
      //console.error('mq broadcastbroadcast %j ',package);
      self.store.broadcast(serverType,package);
      return;
  });
};
...
pro.broadcastByIds = function(err,resp,serverId,toIds,session,req) {
  ...
  this.afterFilter(err, req, _session,resp,function(err,res){
      ...
      self.store.response(serverId,package);
      return;
  });
};
...
pro.doHandle = function(session,msg,routeRecord, cb) {
  ...
  var handle = function(err, resp) {
    ...
    var __msg = self.decode('handle',msg);
    self.handlerService.handle(routeRecord, __msg, session, cb);
  };  //end of handle
